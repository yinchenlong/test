...
@Component({
    selector: 'app-reset-result-tody',
    templateUrl: './reset-result-tody.component.html',
    styleUrls: ['./reset-result-tody.component.css']
})
export class ResetResultTodyComponent implements OnInit {
    // 数据请求的地址
    url = '';
    ...
    getTimeFun(): void {
        ...
        if (this.meType === 'history') {
            // 前一天时间
            this.startTime = new Date(this.endTime.getTime() - 24 * 60 * 60 * 1000);
        } else {
            // 画面类型为当日则当天0点
    ...
    queryActiveAlarmTableListFun(neCode: string, startTime: string, endTime: string): void {
        ...
        this.jsonp.get(this.url + '/wakeUpBts/getActiveAlarmList?neCode=' + neCode + '&startTime=' + startTime +
            '&endTime=' + endTime + '&page=' + _that.activeAlarmPage + '&pageSize=' + _that.activeAlarmPageSize +
            '&callback=JSONP_CALLBACK')
            .subscribe(
                function (data) {
                    const result = data['_body'];
    ...
    querySleepingCellListTableListFun(neCode: string, startTime: string, endTime: string): void {
        ...
        this.jsonp.get(this.url + '/wakeUpBts/getSleepingCellList?neCode=' + neCode + '&startTime=' + startTime +
            '&endTime=' + endTime + '&page=' + _that.sleepingCellPage + '&pageSize=' + _that.sleepingCellPageSize +
            '&callback=JSONP_CALLBACK')
            .subscribe(
                function (data) {
                    const result = data['_body'];
