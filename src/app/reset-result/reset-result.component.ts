...
@Component({
    selector: 'app-reset-result',
    templateUrl: './reset-result.component.html',
    styleUrls: ['./reset-result.component.css']
})
export class ResetResultComponent implements OnInit {
    // 数据请求的地址
    url = '';
    ...
    queryTableDataList(): void {
        ...
        // 画面类型为当日，开始和结束时间只能为当天
        if (this.meType !== 'history') {
            const nowTime = _that.dateToStringFun(new Date());
            if (nowTime.substring(0, 10) !== sTime.substring(0, 10) || nowTime.substring(0, 10) !== eTime.substring(0, 10)) {
                const timeTodayLimitInfo = document.getElementById('timeTodayLimitInfo').innerText;
                _that.message.warning(timeTodayLimitInfo);
                return;
            }
        }

        _that.tableLoading = true;
        this.jsonp.get(this.url + '/wakeUpBts/getWakeUpResultList?neCode=&exceptionType=' + _that.exceptionValue +
            '&cityCode=' + _that.cityCode + '&areaCode=' + _that.regionCode + '&neName=' + _that.neName + '&neVersion=' + _that.version +
            '&startTime=' + sTime + '&endTime=' + eTime + '&meType=' + _that.meType + '&page=' + _that.tablePage + '&pageSize=' +
    ...
    queryActiveAlarmTableListFun(neCode: string, startTime: string, endTime: string): void {
        ...
        this.jsonp.get(this.url + '/wakeUpBts/getActiveAlarmList?neCode=' + neCode + '&startTime=' + startTime +
            '&endTime=' + endTime + '&page=' + _that.activeAlarmPage + '&pageSize=' +
            _that.activeAlarmPageSize + '&callback=JSONP_CALLBACK')
            .subscribe(
                function (data) {
                    const result = data['_body'];
                    if (result['success']) {
    ...
    querySleepingCellListTableListFun(neCode: string, startTime: string, endTime: string): void {
        ...
        this.jsonp.get(this.url + '/wakeUpBts/getSleepingCellList?neCode=' + neCode + '&startTime=' + startTime +
            '&endTime=' + endTime + '&page=' + _that.sleepingCellPage + '&pageSize=' +
            _that.sleepingCellPageSize + '&callback=JSONP_CALLBACK')
            .subscribe(
                function (data) {
                    const result = data['_body'];
                    if (result['success']) {
